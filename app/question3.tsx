...
export default function InterviewQuestion3() {
    ...
    const options = [
        ...
        "Bi-weekly",
        "Monthly",
        "Irregularly / Freelance"
    ];

    return (
        <SafeAreaView className="flex-1 bg-slate-900 px-8 pt-6 pb-12">
            {/* Top Bar */}
            <Pressable onPress={() => router.replace("/question2" as any)} className="w-10 h-10 bg-slate-800 rounded-full items-center justify-center border border-slate-700 mb-6 active:bg-slate-700">
                <Ionicons name="arrow-back" size={20} color="white" />
            </Pressable>

            {/* Header Section */}
            <View>
                <Text className="text-[#94A3B8] text-sm font-bold mb-4 uppercase tracking-widest">Question 3 of 5</Text>
                <Text className="text-white text-4xl font-bold leading-tight mb-2">
                    How often do you usually receive income?
                </Text>
            ...
            <View className="flex-row justify-between items-center mt-4">
                ...
                <Pressable
                    onPress={() => selected && router.replace("/question4" as any)}
