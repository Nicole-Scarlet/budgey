...
const ProgressRing = ({ category, transactions }: { category: any, transactions: any[] }) => {
  ...
  const categoryExpenses = transactions
    .filter(t => t.categoryId === category.id || t.category === category.name) // fallback for legacy mock
...
export default function HomeScreen() {
  ...
  const currentData = {
    title: 'Summary',
    items: sortedTransactions.map((tx: any) => {
      const { prefix, color } = formatTransactionStyles(tx);
      const cat = categories.find(c => c.id === tx.categoryId || c.name === tx.category); // Fallback for legacy
      return {
        label: tx.title,
  ...
  return (
    <View className="flex-1 bg-[#242E42]">
      ...
      <View className="bg-[#303E55] w-full rounded-b-[40px] pt-16 pb-8 px-10 items-center mb-6 shadow-md z-10 relative">
        ...
        <TouchableOpacity onPress={() => router.push('/enter-budget')}>
      ...
      <View className="flex-1">
        {activeModule === 'Summary' ? (
          ...
          >
            {categories.filter(c => c.type === activeModule).length === 0 && (
            ...
            {categories.filter(c => c.type === activeModule).map((sub: any, idx: number) => {
              const isActive = selectedCategory === sub.name;

              // Compute math warnings automatically based on relevant transactions
              const moduleSpecificTransactions = transactions.filter(t => t.type === activeModule);
              const isWarningMode = moduleSpecificTransactions.filter(t => t.categoryId === sub.id).reduce((sum, t) => sum + Number(t.amount), 0) >= ((sub.limit || 1) * 0.9);
      ...
      {/* Persistent BottomSheet for active module Summary */}
      {
        activeModule !== 'Summary' && (
          <BottomSheet
            ref={bottomSheetRef}
          ...
          >
            {/* Extended background view to prevent white color bleed on overscroll */}
            <View className="absolute top-0 left-0 right-0 h-[2000px] w-full bg-[#1E293B] rounded-t-[40px] -z-10" />

            <View className="px-8 pb-4 pt-2">
              <View className="flex-row justify-between items-start mb-2">
                <View className="flex-col">
                  <Text className="text-white text-[24px] font-bold mb-1">
                    {activeModule === 'Expense' ? 'Expenses' : activeModule === 'Investment' ? 'Investments' : activeModule === 'Savings' ? 'Savings' : 'Income'}
                  </Text>
                  <Text className="text-slate-300 text-[14px] font-bold">
                    {new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                  </Text>
                </View>
                <TouchableOpacity onPress={() => router.push({ pathname: '/add-expense', params: { module: activeModule } } as any)}>
                  <Feather name="plus" size={24} color="#E2E8F0" />
                </TouchableOpacity>
              </View>
            </View>

            <BottomSheetFlatList
              data={moduleTransactions}
              keyExtractor={(_: any, idx: number) => idx.toString()}
              contentContainerStyle={{ paddingHorizontal: 32, paddingBottom: 80 }}
              showsVerticalScrollIndicator={false}
              ListEmptyComponent={<Text className="text-slate-400 text-center mt-6">No {activeModule.toLowerCase()}s found.</Text>}
              renderItem={({ item }: { item: any }) => (
                <View className="flex-row justify-between items-center mb-6 mt-4">
                  <View className="flex-row items-center">
                    <Text className="text-slate-200 text-[16px] ml-2">{item.label}</Text>
                  </View>
                  <Text className={`text-[16px] font-bold flex-shrink-0 ml-2 ${item.color}`}>{item.value}</Text>
                </View>
              )}
