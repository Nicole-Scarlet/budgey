...
export default function IncomeScreen() {
    ...
    const [tempPeriod, setTempPeriod] = useState<'Daily' | 'Weekly' | 'Monthly'>(incomeGoalPeriod || 'Monthly');
    ...
    const navCategories = [
        ...
        { name: 'Income', icon: 'wallet-outline', type: 'material', active: true },
    ...
    const incomeTransactions = getTransactionsByType('income').filter(t => isTransactionInActiveFilter(t.date));
        ...
        <SafeAreaView className="flex-1 bg-[#1E293B]" onLayout={(e) => setContainerHeight(e.nativeEvent.layout.height)}>
            <View onLayout={(e) => setHeaderHeight(e.nativeEvent.layout.height)}>
                ...
                <View className="bg-[#334155] rounded-b-[40px] pt-8 pb-10 shadow-lg relative z-50">
                        ...
                        {isCategoryMenuVisible && (
                            ...
                            >
                                <Pressable
                                    onPress={() => {
                                        setIsCategoryMenuVisible(false);
                                        router.push({ pathname: '/add-category', params: { module: 'Income' } } as any);
                                    }}
                                ...
                                <Pressable
                                    onPress={() => {
                                        setIsCategoryMenuVisible(false);
                                        router.push({ pathname: '/category-list', params: { module: 'Income' } } as any);
                                    }}
                    ...
                    >
                        <Text className="text-white text-[42px] font-bold tracking-tight">₱{totalIncome.toLocaleString('en-US', { minimumFractionDigits: 2 })}</Text>
                        <Text className="text-white/80 text-lg font-medium mt-2 mb-2">Overall Income</Text>
                        {incomeGoal > 0 ? (
                            <Pressable onPress={() => { setIsCategoryMenuVisible(false); setTempGoal(incomeGoal.toString()); setTempPeriod(incomeGoalPeriod); setIsGoalModalVisible(true); }} className="bg-[#1E293B] px-4 py-2 rounded-full border border-[#90A1B9]/30">
                            ...
                            <Pressable onPress={() => { setIsCategoryMenuVisible(false); setTempGoal(''); setTempPeriod('Monthly'); setIsGoalModalVisible(true); }} className="bg-[#1E293B] px-4 py-2 rounded-full border border-[#90A1B9]/30">
                                <Text className="text-[#4ADE80] font-medium text-sm">+ Set Income Goal</Text>
                            </Pressable>
                        )}
                    </Pressable>
                </View>
            ...
            <ScrollView className="px-6 flex-1" contentContainerStyle={{ paddingBottom: 250 }} showsVerticalScrollIndicator={false}>
                <Pressable onPress={() => setIsCategoryMenuVisible(false)}>
                    {globalCategories.filter(c => c.type === 'Income').length === 0 ? (
                        ...
                        globalCategories.filter(c => c.type === 'Income').map((cat) => {
                            const categoryTransactions = incomeTransactions.filter(t => t.categoryId === cat.id);
                            const categoryEarned = categoryTransactions.reduce((sum, t) => sum + t.amount, 0);

                            return (
                                <Pressable
                                    key={cat.id}
                                    onPress={() => router.push({ pathname: '/add-expense', params: { category: cat.id, categoryName: cat.name, module: 'Income' } })}
                                    ...
                                    <View className="flex-row items-center flex-1">
                                        ...
                                        <View>
                                            ...
                                            <Text className="text-[#94A3B8] text-[14px]">
                                                Earned: ₱{categoryEarned.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                            </Text>
                                        </View>
                                    </View>
            ...
            {/* Bottom Sheet for Income Summary */}
            <BottomSheet
                ref={bottomSheetRef}
                index={1}
                ...
                <View className="px-6 pt-4 pb-2 flex-row justify-between items-center relative z-50">
                    <View>
                        <View className="flex-row items-center mb-1">
                            <Text className="text-white text-2xl font-bold font-['Inter_700Bold'] mr-3">Income</Text>
                            ...
                            >
                                ...
                                >
                                    <View
                                        ...
                                        style={{ top: height - firstSnap - 10, left: 125 }}
                                    >
                    ...
                    <Pressable onPress={() => router.push({ pathname: '/add-expense', params: { module: 'Income' } })} className="p-2">
                ...
                <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40, paddingTop: 20 }}>
                    {incomeTransactions.length === 0 ? (
                        <View className="items-center justify-center pt-10 pb-20">
                            ...
                            <Text className="text-[#94A3B8] text-lg mt-4 text-center px-4">No income recorded yet.</Text>
                        </View>
                    ) : (
                        incomeTransactions.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <Feather name={(category?.icon as any) || "activity"} size={18} color={category?.color || "#4ADE80"} />
                                    </View>
                ...
                >
                    <Pressable onPress={() => { }} className="w-full">
                        <View className="bg-[#1E293B] w-full rounded-3xl p-6 border border-[#334155] shadow-2xl">
                            <Text className="text-white text-xl font-bold mb-4">Set Income Goal</Text>
                            <Text className="text-slate-400 text-sm mb-4">Enter your target income amount below:</Text>
                                ...
                                <Pressable
                                    onPress={() => {
                                        ...
                                        if (!isNaN(parsedGoal) && parsedGoal > 0 && parsedGoal <= 1000000000) {
                                            setIncomeGoal(parsedGoal);
                                            setIncomeGoalPeriod(tempPeriod);
                                            setIsGoalModalVisible(false);
                                        } else if (tempGoal === '' || parsedGoal === 0) {
                                            setIncomeGoal(0); // clear if empty
                                            setIsGoalModalVisible(false);
                                        }
                                    }}
                                    disabled={parseFloat(tempGoal.replace(/[^0-9.-]+/g, '')) > 1000000000}
                                    className={`px-6 py-3 rounded-xl ${parseFloat(tempGoal.replace(/[^0-9.-]+/g, '')) > 1000000000 ? 'bg-gray-500' : 'bg-[#4ADE80]'}`}
                                >
                                    <Text className={`${parseFloat(tempGoal.replace(/[^0-9.-]+/g, '')) > 1000000000 ? 'text-white' : 'text-[#0F172A]'} font-bold text-center`}>Save Goal</Text>
                                </Pressable>
                            </View>
                        </View>
                    </Pressable>
