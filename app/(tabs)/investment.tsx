        ...
        <SafeAreaView className="flex-1" style={{ backgroundColor: colors.background }} onLayout={(e) => setContainerHeight(e.nativeEvent.layout.height)}>
            <View onLayout={(e) => setHeaderHeight(e.nativeEvent.layout.height)}>
                ...
                >
                        ...
                        {isCategoryMenuVisible && (
                            ...
                            >
                                <Pressable
                                    onPress={() => {
                                        setIsCategoryMenuVisible(false);
                                        router.push({ pathname: '/add-category', params: { module: 'Investment' } } as any);
                                    }}
                                ...
                                <Pressable
                                    onPress={() => {
                                        setIsCategoryMenuVisible(false);
                                        router.push({ pathname: '/category-list', params: { module: 'Investment' } } as any);
                                    }}
                    ...
                    >
                        <Text className="text-[42px] font-bold tracking-tight" style={{ color: colors.foreground }}>{formatCurrency(totalInvestment)}</Text>
                        <Text className="text-lg font-medium mt-2 mb-2" style={{ color: colors.foreground + 'CC' }}>Overall Investment</Text>
                        {investmentLimit > 0 ? (
                            <Pressable 
                                onPress={() => { setIsCategoryMenuVisible(false); setTempLimit(investmentLimit.toString()); setTempPeriod(investmentLimitPeriod); setIsLimitModalVisible(true); }} 
            ...
            <ScrollView className="px-6 flex-1" contentContainerStyle={{ paddingBottom: 250 }} showsVerticalScrollIndicator={false}>
                <Pressable onPress={() => setIsCategoryMenuVisible(false)}>
                    {categories.filter(c => c.type === 'Investment').length === 0 ? (
                        ...
                        categories.filter(c => c.type === 'Investment').map((cat) => {
                            const categoryTransactions = investmentTransactions.filter(t => t.categoryId === cat.id);
                            const categoryInvested = categoryTransactions.reduce((sum, t) => sum + t.amount, 0);

                            return (
                                <Pressable
                                    key={cat.id}
                                    onPress={() => router.push({ pathname: '/add-expense', params: { category: cat.id, categoryName: cat.name, module: 'Investment' } })}
                                    ...
                                    <View className="flex-row items-center flex-1">
                                        ...
                                        <View>
                                            ...
                                            <Text className="text-[14px]" style={{ color: colors.muted }}>
                                                Invested: ₱{categoryInvested.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                            </Text>
                                        </View>
                                    </View>
                ...
                <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 40, paddingTop: 20 }}>
                    {investmentTransactions.length === 0 ? (
                        <View className="items-center justify-center pt-10 pb-20">
                            ...
                            <Text className="text-lg mt-4 text-center px-4" style={{ color: colors.muted }}>No investments recorded yet.</Text>
                        </View>
                    ) : (
                        investmentTransactions.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <Feather name={(category?.icon as any) || "activity"} size={18} color={category?.color || "#A855F7"} />
                                    </View>
                                    ...
                                    <View className="flex-1 justify-center">
                                        ...
                                        <Text className="text-[12px]" style={{ color: colors.muted }}>
                                            {item.date}{activeGroupId ? ` • ${item.userId === currentUserId ? 'You' : (profiles.find(p => p.id === item.userId)?.firstName || 'Member')}` : ''}
                                        </Text>
                                    </View>
            ...
            >
                ...
                >
                    <Pressable onPress={() => { }} className="w-full">
                        ...
                        >
                            <Text className="text-xl font-bold mb-4" style={{ color: colors.foreground }}>Set Investment Goal</Text>
                            <Text className="text-sm mb-4" style={{ color: colors.muted }}>Enter your target investment goal below:</Text>
                            ...
                            <View 
                                ...
                                style={{ backgroundColor: colors.background, borderColor: parseFloat(tempLimit.replace(/[^0-9.-]+/g, '')) > 1000000000 ? '#ef4444' : colors.border }}
                            >
                                ...
                                <Pressable
                                    onPress={async () => {
                                        ...
                                        if (!isNaN(parsedLimit) && parsedLimit > 0 && parsedLimit <= 1000000000) {
                                            await setInvestmentLimit(parsedLimit);
                                            await setInvestmentLimitPeriod(tempPeriod);
                                            setIsLimitModalVisible(false);
                                        } else if (tempLimit === '' || parsedLimit === 0) {
                                            await setInvestmentLimit(0);
                                            setIsLimitModalVisible(false);
                                        }
                                    }}
                                    disabled={parseFloat(tempLimit.replace(/[^0-9.-]+/g, '')) > 1000000000}
                                    className={`px-6 py-3 rounded-xl ${parseFloat(tempLimit.replace(/[^0-9.-]+/g, '')) > 1000000000 ? 'bg-gray-500' : 'bg-[#A855F7]'}`}
                                >
